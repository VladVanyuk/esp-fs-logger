...
bool AsyncFsWebServer::init(AwsEventHandler wsHandle) {
    ...
    if (!file) {
        log_error("Failed to open /config directory. Create new folder\n");
    ...
    if (!file) {
        ...
        if(!file)
        {
            ...
            m_filesystem_ok = false;
        }
        file.print("{\"wifi-box\": \"\",\n\t\"dhcp\": false}"); // TODO ADAPT  
        file.close();
    } else {
        file.close();
    }
...
#if ESP_FS_WS_LOGS
    File fileLogs = m_filesystem->open(ESP_FS_WS_LOGS_FOLDER, "r");
    if (!fileLogs) {
        log_error("Failed to open /logs directory. Create new folder\n");
        m_filesystem->mkdir(ESP_FS_WS_LOGS_FOLDER); // TODO  CHECK AND TEST
    }

    // Check if config file exist, and create if necessary
    fileLogs = m_filesystem->open(ESP_FS_WS_LOGS_FILE, "r");
    if (!fileLogs) {
        fileLogs = m_filesystem->open(ESP_FS_WS_LOGS_FILE, "w");
        if(!fileLogs)
        {
            log_error("Failed to create general logs file.\n");
            m_filesystem_ok = false;
        }
        fileLogs.println(__DATE__);
        fileLogs.print(__TIME__);
    ...
    on("/status", HTTP_GET, std::bind(&AsyncFsWebServer::getStatus, this, _1));
    ...
    serveStatic("/", *m_filesystem, "/").setDefaultFile("setup.htm"); //TODO TEST @VladVanyuk
    ...
    MDNS.setInstanceName("async-fs-webserver"); // todo recode maybe? 
    return true;
}
...
void AsyncFsWebServer::enableFsCodeEditor() {
    ...
    on("/edit", HTTP_GET, std::bind(&AsyncFsWebServer::handleFileEdit, this, _1));// todo analyze 
    on("/edit", HTTP_POST,
        std::bind(&AsyncFsWebServer::sendOK, this, _1),
        std::bind(&AsyncFsWebServer::handleUpload, this, _1, _2, _3, _4, _5, _6)
    );
...
void AsyncFsWebServer::onUpdate(size_t& contentLen) {
        ...
        if (final) {
            ...
            if (!Update.end(true)) { //true to set the size to the current progress
                log_error("%s\n", Update.errorString());    
            //    setTaskWdt(m_watchdogTime); //TODO!!!!
...
void AsyncFsWebServer::handleSetup(AsyncWebServerRequest *request) {
    ...
    // Changed array name to match SEGGER Bin2C output @bogdan9876
...
#endif
    doc["liburl"] = "https://vujko.net/";
    //todo add wthat in status request
    String reply;
    ...
    */
    ...
    if (ssid.length() && pass.length()) {
        ...
        while (WiFi.status() != WL_CONNECTED) {
            ...
            Serial.print("*");
            #if defined(ESP8266)
            ESP.wdtFeed(); // todo @VladVanyuk check with original lib
            #else
            esp_task_wdt_reset();
            #endif
...
void  AsyncFsWebServer::update_first(AsyncWebServerRequest *request, String filename, size_t index, uint8_t *data, size_t len, bool final) {
    ...
    if (!index) {
        ...
        int32_t maxSketchSpace = (ESP.getFreeSketchSpace() - 0x1000) & 0xFFFFF000;
 ...
 * 
 * @details 
 *  1. startCaptivePortal 
 *  2. returns ip = m_captiveIp 
 */
IPAddress AsyncFsWebServer::startWiFi(uint32_t timeout, CallbackF fn, bool skipAP) {
    ...
    if (!ip) {
        if (!m_apSSID.length()) {
            char _ssid[32];
            #ifdef ESP8266
            snprintf(_ssid, sizeof(_ssid), "ESP-%d", ESP.getChipId());
            #elif defined(ESP32)
            snprintf(_ssid, sizeof(_ssid), "ESP32-%llX", ESP.getEfuseMac());
            #endif
            m_apSSID = _ssid;
        }
...
void AsyncFsWebServer::handleFsStatus(AsyncWebServerRequest *request)
{
    ...
    fsInfo_t info = {1024, 1024, "Vujko Filesystem"};
#ifdef ESP8266
    FSInfo fs_info;
    ...
    json += info.fsName;
    json += "\", \"isOk\":";
    // #ifdef (VUJKO_FIRMWARE_VERSION)
    json += "\", \"version\":";
    json += String(__DATE__) + String('_') + String(__TIME__);
    // #endif
    if (m_filesystem_ok)  {
        // IPAddress ip = (WiFi.status() == WL_CONNECTED) ? WiFi.localIP() : WiFi.softAPIP();
        #if LOG_LEVEL > 0
        IPAddress ip = WiFi.softAPIP();
        log_debug("IP address: %s", ip.toString().c_str());
        #else
        WiFi.softAPIP(); // TODO delete and test
        #endif
        ...
        json += PSTR("\", \"ssid\":\"");
        json += WiFi.SSID();
        json += PSTR("\", \"date\":\""); // json += ip.toString();
        json += __DATE__;
        json += PSTR("\", \"time\":\""); 
        json += __TIME__;
        json += "\"";
